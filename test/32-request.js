...
describe('request', () => {
  ...
  it('should create query object based on request query', async () => {
    ...
    const body = await fetch(url + '/query?a=1&b=2').then(o => o.text())
    assert.strictEqual(body, JSON.stringify({ a: '1', b: '2'}))
  })
  
  it('should create params object based on url params', async () => {
    server.get('/params/:p1/static/:p2', async req => req.params)
    const body = await fetch(url + '/params/value1/static/value2').then(o => o.text())
    assert.strictEqual(body, JSON.stringify({ p1: 'value1', p2: 'value2' }))
  })
  ...
  it('should parse payload: json', async () => {
    ...
    assert.ok(!Number.isInteger(resp.headers.get('content-length')), 'header "content-length" is not a number')
