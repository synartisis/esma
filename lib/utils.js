 ...
 * @param {keyof typeof httpRedirectStatusCodes} statusCode http status code (default 302)
 * @returns {esma.HandlerResultHttpObject}
 * @example
 * server.get('/a-path', req => {
 *   return esma.httpRedirect('/')  // back to root
 * })
 */
export function httpRedirect(location, statusCode = 302) {
  return { $statusCode: statusCode, $headers: { 'Location': location }, $body: '' }
}
...
const httpRedirectStatusCodes = /** @type {const} */({
