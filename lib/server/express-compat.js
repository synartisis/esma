...
/** @type {(httpRequest: http.IncomingMessage, ctx: Context) => Promise<Request>} */
export async function patchRequest(httpRequest, ctx) {
  ...
  if (['POST', 'PUT', 'PATCH'].includes(method)) req.body = await parseRequestBody(req, ctx)
  return req
}
