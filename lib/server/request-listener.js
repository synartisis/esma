...
function finalHandler(req, res, ctx, result) {
  ...
  if (typeof result === 'object' && getObjectType(result) === 'object') {
    // const { $statusCode, $headers, $body, ...rest } = result
    if ('$statusCode' in result && typeof result.$statusCode === 'number') res.statusCode = result.$statusCode
    if ('$headers' in result && typeof result.$headers === 'object' && !!result.$headers) Object.entries(result.$headers).forEach(([k, v]) => res.setHeader(k, v))
