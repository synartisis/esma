...
function createMiddlewareEntry(path, handlers, { method = '*', router, mounting = false, mountpath }) {
  ...
  const mdlEntry = {
    ...
    async applyHandlers(req, res, ctx) {
      if (!allowedMethod(req, method)) return null
      ...
      for await (const handler of this.handlers) {
        ...
        if (typeof handler === 'function') {
          ...
          try {
            result = await handler(req, res, next.bind(ctx))
          } catch (error) {
            console.log({error})
...
function allowedMethod(req, method) {
...
function next(kind) {
  ...
  if (['route', 'router'].includes(kind)) this.express_action = kind
}
