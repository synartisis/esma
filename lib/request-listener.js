...
/** @type {(req: esma.Request, res: esma.Response, ctx: esma.Context, result: esma.HandlerResult) => void} */
function finalHandler(req, res, ctx, result) {
  ...
  if (typeof result === 'object') {
    ...
    if ($statusCode || $headers || '$body' in result) {
      if (typeof $statusCode == 'number') res.statusCode = $statusCode
      if (!!$headers && typeof $headers === 'object') {
        Object.entries($headers).forEach(([k, v]) => res.setHeader(k, v))
      }
      if (!('$body' in result)) throw new HttpError(500, `$body property is required when $statusCode or $headers are set`, req, res)
