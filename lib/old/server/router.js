...
import { addHandler } from '../middleware/handlers.js'
  ...
  const r = {
    ...
    use(path, ...handlers) {
      ...
      for (const handler of handlers) {
        ...
        addHandler((this.mountpath + path).replaceAll('//', '/'), '*', handler)
      }

      const mountpath = (this.mountpath + pathWithoutTrailingSlash).replaceAll('//', '/')
      // const mdlEntry = createMiddlewareEntry(mountpath, handlers, { router: this, mounting: true, mountpath })
      const mdlEntry = createMiddlewareEntry(path, handlers, { router: this, mounting: true, mountpath })
    ...
    _method(method, path, handlers) {
      ...
      for (const handler of handlers) {
        if (typeof handler !== 'function') continue
        addHandler((this.mountpath + path).replaceAll('//', '/'), method, handler)
      }
...
/** @type {(path: string, handlers: esma.Handler[], options: any) => esma.MiddlewareEntry} */
function createMiddlewareEntry(path, handlers, { method = '*', router, mounting = false, mountpath }) {
  ...
  handlers.forEach(h => {
    // if h.length === 4, h works as an express error handler
    // @ts-ignore
  ...
  const mdlEntry = {
    ...
    async applyHandlers(req, res, ctx) {
      ...
      if (mounting) {
        if (this.path !== '/') {
          oldReqUrl = req.url
          // req.url = this.router.mountpath !== '/' ? req.originalUrl.replace(this.router.mountpath, '') ?? '/' : req.originalUrl
...
/** @type {(req: esma.Request<unknown>, method: esma.HttpMethods | '*') => boolean} */
...
/** @type {(req: esma.Request<unknown>, reqData: { params: Record<string, string>, query: Record<string, string> }) => void} */
