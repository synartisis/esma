...
export function serveStatic(root, options) {
  ...
  return async function middlewareStatic(req, res) {
    ...
    if (fileParsers.length > 0) {
      ...
      for (const parser of fileParsers) {
        ...
        const context = { ...res.locals, ...req.view }
        content = await parser(content, filename, context)
      }
      ...
      let stream
      if (req.headers.range) {    // support range requests
        ...
        if (start > end || start >= totalSize || end >= totalSize) {
          return { $statusCode: 416, $headers: { 'Content-Range': `bytes */${totalSize}` }, $body: null }
        }
        res.statusCode = 206
