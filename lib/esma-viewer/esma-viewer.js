...
function routes(req, res) {
  const style = `table {border-collapse: collapse;} td {white-space:pre;font-family:'Courier New';border:solid 1px #ccc;vertical-align:top;padding:.3rem;}`
  return res.end(htmlIt(res, `<style>${style}</style>` + '<table style="white-space:pre-line"><tr><td>' + handlers.map(o => [o.method, o.path, o.handler.toString().substring(0, 250)].join('</td><td>')).join('</td></tr><tr><td>') + '</td></tr></table>'))
}
