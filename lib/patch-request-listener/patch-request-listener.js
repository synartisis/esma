...
/** @type {(httpRequest: http.IncomingMessage, httpResponse: http.ServerResponse, ctx: esma.Context) => { reqEsma: esma.Request, resEsma: esma.Response }} */
...
/** @type {(httpResponse: http.ServerResponse, reqEsma: esma.Request, ctx: esma.Context) => esma.Response} */
function patchResponse(httpResponse, reqEsma, ctx) {
  const resExpress = expressPatchResponse(httpResponse, ctx)
  const resEsma = esmaPatchResponse(resExpress, reqEsma, ctx)
  return resEsma
}