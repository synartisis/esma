  ...
  const r = {
    ...
    _method(method, path, handlers) {
      ...
      for (const handler of handlers) {
        ...
        this.handlers.push({ method, path, handler })
        if (this.mount) {
          addHandler(joinpaths(this.mount, path), method, this.mount, handler)
        }
      }
    },
...
export function mountRouter(router) {
  ...
  for (const handler of router.handlers) {
    const handlerPath = joinpaths(router.mount, handler.path)
    const mountPath = handler.method === '*' ? handlerPath : router.mount
    addHandler(handlerPath, handler.method, mountPath, handler.handler)
  }
