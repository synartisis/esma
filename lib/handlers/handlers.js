...
/** @type {(req: esma.Request, res: esma.Response, ctx: types.Context) => Promise<esma.HandlerResult>} */
export async function handleRequest(req, res, ctx) {
  ...
  for await (const handlerEntry of handlers) {
    ...
    if (handlerEntry.method !== '*') endpointFound = true  // at least one handler matched the request
  ...
  if (endpointFound) {
    ...
    if (result === undefined && res.writableLength === 0) result = null
  }
  // if its a piped response, wait for stream to finish
